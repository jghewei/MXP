...
static int handle_Eth(netsnmp_mib_handler *handler, netsnmp_handler_registration *reginfo, netsnmp_agent_request_info *reqinfo, netsnmp_request_info *requests)
{
...
void InitEthMib(void)
{
    const oid Eth_oid[] = { 1, 3, 6, 1, 4, 1, 19293, 2, 10, 1 };
    table_set = netsnmp_create_table_data_set("Eth");
    table_set->allow_creation = 1;
    netsnmp_table_set_add_indexes(table_set, ASN_INTEGER, 0);
    netsnmp_table_set_multi_add_default_row(table_set,
                                            1, ASN_OCTET_STR, 0, NULL, 0,
                                            0);
    netsnmp_register_table_data_set(netsnmp_create_handler_registration("Eth", handle_Eth, Eth_oid, OID_LENGTH(Eth_oid), HANDLER_CAN_RWRITE), table_set, NULL);
