...
def gen_page_cpp(mname, enum_list, atrb_list):
    ...
    for atrb in atrb_list:
        ...
        if atrb.type == 'enum' or atrb.type == 'int':
            f.write('        pMob->' + atrb.name + '.Set(atoi(ptr));\n')
        elif atrb.type == 'float':
            f.write('        pMob->' + atrb.name + '.Set(atof(ptr));\n')
        else:
            f.write('        pMob->' + atrb.name + '.Set(ptr);\n')
        f.write('    }\n')
        f.write('\n')
    f.write('    pMob->Save();\n')
    f.write('\n')
...
#        f.write('            APP_LOG_LINE();\n')
    ...
    for atrb in atrb_list:
        ...
        f.write('    const oid ' + atrb.name + '_oid[] = { 1, 3, 6, 1, 4, 1, 19293, 2, ' + str(mibIndex) + ', ' + str(entryIndex) + ' };\n')
...
def gen_snmpTable_cpp(mname, enum_list, atrb_list):
    ...
    f.write('void Init' + mname + 'Mib(void)\n')
    f.write('{\n')
    f.write('    const oid ' + mname + '_oid[] = { 1, 3, 6, 1, 4, 1, 19293, 2, ' + str(mibIndex) + ', 1 };\n')
